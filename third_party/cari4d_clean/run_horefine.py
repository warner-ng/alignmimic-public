...
class HORefineRunner(BehaveFPNLFRenderer):
    ...
    def run_1seq(self, args, cfg, evaluator, trainer, errors_all, frames_all):
        ...
        if tex is None:
            tex_map = torch.ones((1, 2, 2, 3), device=device)
            uv = torch.zeros((meshes[obj_idx].verts_padded()[0].shape[0], 2), device=device)
            uv_idx = meshes[obj_idx].faces_padded()[0]
        else:
            tex_map = tex.maps_padded().to(device).float()
            uv = tex.verts_uvs_padded()[0]
            uv[:, 1] = 1 - uv[:, 1]
            uv_idx = tex.faces_uvs_padded()[0]
        mesh_tensors_obj = {
            "tex": tex_map,  # this must be (1, H, W, 3)
            'uv_idx': torch.tensor(uv_idx, device=device, dtype=torch.int),
